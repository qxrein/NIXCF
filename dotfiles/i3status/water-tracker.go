...
const (
	configFile      = "/tmp/i3status-water-tracker.json"
	totalDailyWater = 3700 // in ml (3.7 liters recommended)
)
...
func main() {
	...
	percentage := (state.CurrentLevel * 100) / totalDailyWater
	if percentage < 0 {
		percentage = 0
	}

	output := map[string]interface{}{
		"full_text":   fmt.Sprintf("💧 %d%%", percentage),
		"color":       getColor(percentage),
		"name":        "water",
		"instance":    "water",
		"urgent":      percentage < 20,
	}
